...
export const ModernClassicTemplate: React.FC<{ resume: ResumeData }> = ({ resume }) => {
  const { personalInfo, experience, education, skills, projects, languages, certificates } = resume;
  
  return (
    <div className="max-w-4xl mx-auto bg-white shadow-lg">
      <header className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-8 rounded-t-lg">
              ...
              <section className="mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {skills.map((skill: Skill, idx: number) => (
                    <span key={idx} className="px-3 py-1 bg-gray-50 rounded-full text-blue-600">
                      {skill.name}
                    </span>
                  ))}
                </div>
